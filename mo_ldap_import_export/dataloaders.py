...
class DataLoader:
    ...
    async def create_employee(self, obj: Employee) -> UUID:
        ...
        # As RAModels does not fit GraphQL we have to remap some keys
        field_mapper = {"givenname": "given_name"}
        adjusted_ra_dictionary = {
            field_mapper.get(key, key): value for key, value in obj.dict().items()
        }
        result = await self.graphql_client.employee_create(
            EmployeeCreateInput(**adjusted_ra_dictionary)
        )
        return result.uuid
