...
def gen_employee(user_key: str, **kwargs: Any) -> Employee:
    return Employee(user_key=user_key, **kwargs)
...
async def test_create_unknown_type(dataloader: DataLoader) -> None:
    ...
    assert "Unknown MOBase to create" in str(exc_info.value)
...
async def test_create_each_type_model_client(
...
@pytest.mark.parametrize(
    "mutator,obj",
    [
        ("address_create", gen_address("2")),
        ("employee_create", gen_employee("4", givenname="Monto", surname="Gazlowe")),
    ],
)
async def test_create_each_type(
    graphql_mock: GraphQLMocker, dataloader: DataLoader, mutator: str, obj: MOBase
) -> None:
    create_uuid = uuid4()

    route = graphql_mock.query(mutator)
    route.result = {mutator: {"uuid": create_uuid}}
...
async def test_employee_required_fields_mismatch() -> None:
    """Document that RAModels employee does require the same fields as GraphQL."""
    raemployee = gen_employee("4")

    # We can construct an RAModels employee with missing fields in GraphQL
    with pytest.raises(ValidationError) as exc_info:
        EmployeeCreateInput(**raemployee.dict())
    assert "given_name\n  field required" in str(exc_info.value)
    assert "surname\n  none is not an allowed value" in str(exc_info.value)


async def test_employee_givenname_mismatch() -> None:
    """Document that RAModels employees givenname mismatch with GraphQLs."""
    # Givenname does not map to given_name in GraphQL
    raemployee = gen_employee("4", givenname="Othmar", surname="Garithos")
    with pytest.raises(ValidationError) as exc_info:
        EmployeeCreateInput(**raemployee.dict())
    assert "given_name\n  field required" in str(exc_info.value)


async def test_address_required_fields_mismatch() -> None:
    """Document that RAModels address does require the same fields as GraphQL."""
    raaddress = gen_address("2")

    # We can construct an RAModels employee with missing fields in GraphQL
    with pytest.raises(ValidationError) as exc_info:
        AddressCreateInput(**raaddress.dict())
    assert "validity -> from\n  field required" in str(exc_info.value)
    assert "address_type\n  value is not a valid uuid" in str(exc_info.value)
